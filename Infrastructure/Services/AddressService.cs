...
public class AddressService(AddressRepository addressRepository)
{
    ...
    public async Task<ResponseResult> CreateAddressAsync(string streetName, string postalCode, string city)
    {
        try
        {
            var exists = await _addressRepository.AlreadyExistsAsync(x => x.AddressLine1 == streetName && x.PostalCode == postalCode && x.City == city);
            if (exists is null)
            {
    ...
    public async Task<ResponseResult> GetAddressAsync(string streetName, string postalCode, string city)
    {
        try
        {
            var result = await _addressRepository.GetOneAsync(x => x.AddressLine1 == streetName && x.PostalCode == postalCode && x.City == city);
            return result;
        }
        catch (Exception ex) { return ResponseFactory.Error(ex.Message); }
    }
